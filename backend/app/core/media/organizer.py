...
async def _plan(
    ...
    *,
    ...
    Keep nested TV directories containing videos in place for independent
    organization, even when their media items have not been indexed yet.
    Retain the parent NFO when a split season keeps its original NFO path, and
    rename default danmaku caches with their video basenames. Skip flat movies
    sharing a basename and destinations already containing another movie video
    so their shared NFO and subtitles remain available.
    Clear missing NFO and danmaku cache references so later writes use the
    organized media path.
    Include parsed episode metadata in the journal, even when missing template
    fields require keeping the original video filename.
    Reject existing or planned files occupying destination directory paths before
    persisting a journal or moving any files.
