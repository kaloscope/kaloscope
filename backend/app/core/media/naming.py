...
def render_path(template: str, context: dict[str, Any], lib_type: str) -> PurePosixPath:
    ...
    """
    tmpl = validate_template(template, lib_type)
    if tmpl is None:
        raise ValueError("rename template is empty")
    return PurePosixPath(
        *(_render_component(part, context) for part in tmpl.split("/"))
    )
...
def render_directory(
    ...
    if tmpl is None:
        raise ValueError("rename template is empty")
    return PurePosixPath(
        *(_render_component(part, context) for part in tmpl.split("/")[:-1])
    )
...
def render_filename(template: str, context: dict[str, Any], lib_type: str) -> str:
    ...
    if tmpl is None:
        raise ValueError("rename template is empty")
    return _render_component(tmpl.split("/")[-1], context)
