...
def test_upgrade_data():
    async def run():
        ...
        try:
            ...
            new_fields = {
                "media_lib": {"rename_template": None},
                "media_event": {"payload": None},
                "download_task": {"transfer_targets": None, "transfer_pending": 0},
            }
            ...
            for table, columns in new_fields.items():
                for column in columns:
                    assert column not in before[table][0]

            target = MigrationTarget("models", "0007_media_transfer_pending")
            ...
            for table in tables:
                ...
                for column, default in new_fields.get(table, {}).items():
                    assert rows[0].pop(column) == default
            ...
            await connection.execute_query(
                "UPDATE download_task SET transfer_pending = 1 WHERE id = ?", [task.id]
            )
            await executor.migrate([target])
            ...
            await executor.migrate(
                [MigrationTarget("models", "0006_media_organization")]
            )
            rows = await connection.execute_query_dict("SELECT * FROM download_task")
            assert "transfer_pending" not in rows[0]
            assert rows[0].pop("transfer_targets") is None
            assert rows == before["download_task"]
